...
notify_user() {
  ...
  case "$backend" in
    macos-terminal)
      macos_display_notification_from_terminal "$title" "$message" "$state_dir" "$macos_sender_bundle" && return
      ;;
    terminal-notifier)
    ...
    tmux)
      ...
      ;;
    auto)
      if is_macos && macos_display_notification_from_terminal "$title" "$message" "$state_dir" "$macos_sender_bundle"; then
        return
      fi
      ;;
    *)
