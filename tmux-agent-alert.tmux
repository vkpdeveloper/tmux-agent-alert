...
main() {
  ...
  tmux set-hook -g 'after-select-pane[90]' "run-shell -b '$CURRENT_DIR/bin/agent-alert clear-alert #{pane_id} >/dev/null 2>&1 || true'"
  tmux set-hook -g 'after-select-window[90]' "run-shell -b '$CURRENT_DIR/bin/agent-alert clear-alert #{pane_id} >/dev/null 2>&1 || true'"
  tmux set-hook -g 'session-window-changed[90]' "run-shell -b '$CURRENT_DIR/bin/agent-alert clear-alert #{pane_id} >/dev/null 2>&1 || true'"
  tmux set-hook -g 'client-session-changed[90]' "run-shell -b '$CURRENT_DIR/bin/agent-alert clear-alert #{pane_id} >/dev/null 2>&1 || true'"
